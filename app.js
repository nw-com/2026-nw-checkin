...
const netIndicatorHome = document.getElementById("netIndicatorHome");
...
function updateHomeClockOnce() {
  ...
  try {
    ...
    if (h === 0 && m === 0 && !appState.midnightResetDone) {
      try {
        const st = getTodayRosterStatusForUser(appState.currentUserId);
        const msg = (() => {
          switch (st) {
            case '上班日': return '今日上班日尚未打卡';
            case '值班日': return '今日值班日尚未打卡';
            case '休假日': return '今日休假日';
            case '特休日': return '今日特休日';
            case '請假日': return '今日請假日';
            default: return '今日尚未打卡';
          }
        })();
        if (homeStatusEl) homeStatusEl.textContent = msg;
      } catch { if (homeStatusEl) homeStatusEl.textContent = '今日尚未打卡'; }
      try { autoCompleteCheckoutForYesterday(); } catch {}
...
async function autoCompleteCheckoutForYesterday() {
  try {
    ...
    const dayStart = new Date(yest.getFullYear(), yest.getMonth(), yest.getDate());
    const dayEnd = new Date(yest.getFullYear(), yest.getMonth(), yest.getDate()+1);
    const q = fns.query(fns.collection(db, 'checkins'), fns.where('uid','==', user.uid));
    ...
    snap.forEach((doc) => {
      ...
      if (dt >= dayStart && dt < dayEnd) list.push({ ...d, dt });
    });
    const start = list.filter((r) => String(r.status||'').split('-')[0] === '上班').sort((a,b)=> a.dt - b.dt)[0] || null;
    const end = list.filter((r) => String(r.status||'').split('-')[0] === '下班').sort((a,b)=> b.dt - a.dt)[0] || null;
    if (start && !end) {
      const dtStr = `${yest.getFullYear()}-${String(yest.getMonth()+1).padStart(2,'0')}-${String(yest.getDate()).padStart(2,'0')} 17:30:00`;
      const payload = {
        uid: user.uid,
        name: (homeHeaderNameEl?.textContent || '').replace(/^歡迎~\s*/, '') || (user?.email || '使用者'),
        role: appState.currentUserRole || '一般',
        status: '下班',
        locationType: start.locationType || null,
      ...
      try { await withRetry(() => fns.addDoc(fns.collection(db, 'checkins'), payload)); } catch {}
    }
  } catch {}
}
...
async function hasApprovedLeaveToday(uid) {
  try {
    await ensureFirebase();
    const now = nowInTZ('Asia/Taipei');
    const dayStart = new Date(now.getFullYear(), now.getMonth(), now.getDate());
    const dayEnd = new Date(now.getFullYear(), now.getMonth(), now.getDate()+1);
    const monthKey = `${now.getFullYear()}-${now.getMonth()+1}`;
    const cached = appState.rosterMonthLeaves?.[uid]?.[monthKey] || [];
    let hit = Array.isArray(cached) && cached.length ? cached.some((r) => !(r.edt < dayStart || r.sdt >= dayEnd)) : false;
    if (hit) return true;
    const ref = fns.collection(db, 'leaveRequests');
    const q = fns.query(ref, fns.where('uid','==', uid));
    const snap = await withRetry(() => fns.getDocs(q));
    ...
    snap.forEach((doc) => {
      const d = doc.data() || {};
      const st = String(d.status||''); if (st && st !== '核准') return;
      const s = d.startAt; const e = d.endAt;
      const sdt = typeof s === 'string' ? new Date(s) : (s && typeof s.toDate === 'function' ? s.toDate() : null);
      const edt = typeof e === 'string' ? new Date(e) : (e && typeof e.toDate === 'function' ? e.toDate() : null);
      if (!(sdt instanceof Date) || isNaN(sdt) || !(edt instanceof Date) || isNaN(edt)) return;
      if (!(edt < dayStart || sdt >= dayEnd)) ok = true;
    });
    return ok;
  } catch { return false; }
}
...
async function getTodayRosterStatusForUserAsync(uid) {
  try {
    ...
    if (now.getDay() === 5 && !isTWNationalHoliday(now) && base !== '值班日' && base !== '請假日') base = '特休日';
    const u = String(uid||'');
    const leave = await hasApprovedLeaveToday(u);
    if (leave) base = '請假日';
    return base;
  } catch { return getTodayRosterStatusForUser(uid); }
}

async function setHomeNoCheckinMessage() {
  try {
    if (!homeStatusEl) return;
    const uid = appState.currentUserId || auth?.currentUser?.uid || '';
    const st = await getTodayRosterStatusForUserAsync(uid);
    const msg = st === '上班日' ? '今日上班日尚未打卡'
              : st === '值班日' ? '今日值班日尚未打卡'
              : st === '休假日' ? '今日休假日'
              : st === '特休日' ? '今日特休日'
              : st === '請假日' ? '今日請假日'
  ...
  if (userPhotoEl) {
    ...
    userPhotoEl.onclick = () => { try { showProfileModal(user, "一般"); } catch {} };
      ...
      try {
        ...
        } else { await setHomeNoCheckinMessage(); }
      } catch {}
...
function hasFullAccessToTab(tab) {
  ...
  function setActiveSubTab(label) {
    ...
    if (activeMainTab === "checkin") {
      ...
      if (featureSubTitle) featureSubTitle.textContent = label;
    ...
    if (label === "報表") {
      (async () => {
        try {
          ...
          const m = tzNow.getMonth() + 1;
          const todayYm = `${y}-${String(m).padStart(2,'0')}`;
          // 預取本使用者全部打卡與請假資料（後續依月份過濾）
          const refC = fns.collection(db, 'checkins');
          const qC = fns.query(refC, fns.where('uid', '==', user.uid));
          const snapC = await withRetry(() => fns.getDocs(qC));
          const checkins = [];
          snapC.forEach((doc) => {
            try {
              const d = doc.data() || {};
              ...
              if (created && typeof created.toDate === 'function') dt = created.toDate(); else if (typeof created === 'string') dt = new Date(created);
              if (!(dt instanceof Date) || isNaN(dt)) return;
              checkins.push({ id: doc.id, ...d, dt });
            } catch {}
          });
          const refL = fns.collection(db, 'leaveRequests');
          const qL = fns.query(refL, fns.where('uid', '==', user.uid));
          const snapL = await withRetry(() => fns.getDocs(qL));
          const leaves = [];
          snapL.forEach((doc) => {
            try {
              const d = doc.data() || {};
              ...
              const sdt = typeof s === 'string' ? new Date(s) : (s && typeof s.toDate === 'function' ? s.toDate() : null);
              const edt = typeof e === 'string' ? new Date(e) : (e && typeof e.toDate === 'function' ? e.toDate() : null);
              leaves.push({ id: doc.id, ...d, sdt, edt });
            } catch {}
          });
          container.innerHTML = `
            <div class="row" style="justify-content: space-between; align-items: center;">
              <div class="muted">當月出勤紀錄列表</div>
              <button class="btn" id="btnPrintReport">列印</button>
            </div>
            <div class="roster-datebar" style="margin-top: 6px;">
              <input id="reportMonth" type="month" class="input" />
            </div>
            <div class="table-wrapper">
          ...
          const baseStatusForDate = (date) => {
            ...
            let base = rp && rp.status ? norm(rp.status) : defaultRosterStatusForDate(date);
            // 週五一律轉為特休日（排除國定/值班/請假）
            if (isFri && !isTWNationalHoliday(date) && base !== '值班日' && base !== '請假日') base = '特休日';
            return base;
          };
          ...
          const renderMonth = (ymStr) => {
            ...
            for (let d = new Date(start); d < end; d.setDate(d.getDate()+1)) {
              ...
              const outCandidates = dayList.filter((r) => pickBase(r.status)==='外出');
              const endCandidates = dayList.filter((r) => pickBase(r.status)==='下班');
              const startAt = (startCandidates.length ? startCandidates : outCandidates).sort((a,b)=>a.dt-b.dt)[0]?.dt || null;
  ...
  function renderFeatureContent(label) {
    ...
    if (label === '簽呈') {
      container.innerHTML = `
        <div class="block">
          <div class="row" style="justify-content: space-between; align-items: center;">
            <div class="muted">個人簽呈列表</div>
            <button class="btn btn-darkgrey" id="btnAddMemo">新增</button>
          </div>
          <div class="table-wrapper">
        ...
        <div class="block">
          <div class="row" style="justify-content: space-between; align-items: center;">
            <div class="muted">公司人員簽成列表</div>
            <button class="btn btn-darkgrey" id="btnAddMemoOrg" disabled>新增</button>
          </div>
          <div class="table-wrapper">
            <table class="table" id="memoTableOrg">
              <thead>
                <tr>
                  <th>編號</th>
                  <th>主旨</th>
              ...
              <tbody id="memoListOrg"></tbody>
            </table>
          </div>
        </div>
      `;
      const listEl = container.querySelector('#memoList');
      const btnAdd = container.querySelector('#btnAddMemo');
      const listOrgEl = container.querySelector('#memoListOrg');
      const btnAddOrg = container.querySelector('#btnAddMemoOrg');
      const states = ['待簽核','核准','不核准','請補件'];
      const ensureUser = async () => { await ensureFirebase(); return auth?.currentUser || null; };
      const readLocal = (uid) => { try { const s = localStorage.getItem(`memos:${uid}`); return s ? JSON.parse(s) : []; } catch { return []; } };
      const writeLocal = (uid, arr) => { try { localStorage.setItem(`memos:${uid}`, JSON.stringify(arr)); } catch {} };
      const genNumber = async () => {
        const u = await ensureUser(); if (!u) return null;
        const dt = new Date(); const ds = `${dt.getFullYear()}-${String(dt.getMonth()+1).padStart(2,'0')}-${String(dt.getDate()).padStart(2,'0')}`;
        let seq = 1;
        try {
          if (db && fns.collection && fns.getDocs && fns.query && fns.where) {
            const q = fns.query(fns.collection(db, 'memos'), fns.where('dateStr','==',ds));
            const snap = await withRetry(() => fns.getDocs(q));
            seq = (snap?.size || 0) + 1;
          }
        } catch {}
        if (seq === 1) { const arr = readLocal(u.uid).filter((x) => String(x.dateStr||'') === ds); if (arr.length) seq = arr.length + 1; }
        const next = String(seq).padStart(3,'0');
        return `${ds}-${next}`;
      };
      const openViewDoc = (m) => {
        const isPdf = /^data:application\/(pdf)/.test(String(m.fileDataUrl||''));
        const hasImg = /^data:image\//.test(String(m.fileDataUrl||''));
        const content = (!m.fileDataUrl) ? '<p class="muted">尚未上傳文件</p>' : (isPdf ? `<embed src="${m.fileDataUrl}" type="application/pdf" style="width:100%;height:60vh;border:1px solid #ddd;border-radius:6px;"/>` : (hasImg ? `<img src="${m.fileDataUrl}" alt="文件" style="max-width:100%;max-height:60vh;border-radius:6px;border:1px solid #ddd;object-fit:contain;"/>` : `<p class="muted">不支援的檔案格式</p>`));
        openModal({ title: `文件 - ${m.number || ''}`, fields: [], submitText: null, afterRender: ({ body }) => {
          const wrap = document.createElement('div'); wrap.innerHTML = content; body.appendChild(wrap);
        }});
      };
      const addOrEdit = async (m = null, mode = 'new') => {
        const isEdit = mode === 'edit';
        const number = isEdit ? (m?.number || '') : (await genNumber());
        openModal({
          title: isEdit ? '編輯簽呈' : '新增簽呈',
          fields: [
            { key: 'number', label: '編號', type: 'text', readonly: true },
            { key: 'subject', label: '主旨', type: 'text' },
            { key: 'description', label: '說明', type: 'text' },
            { key: 'fileDataUrl', label: '上傳文件', type: 'file', accept: '.pdf,.png,.jpg,.jpeg' },
          ],
          initial: { number, subject: m?.subject || '', description: m?.description || '', fileDataUrl: m?.fileDataUrl || null },
          submitText: '儲存',
          onSubmit: async (data) => {
            try {
              await ensureFirebase(); const u = auth?.currentUser || null; if (!u) { alert('請先登入'); return false; }
              const payload = {
                number: data.number, subject: String(data.subject||'').trim(), description: String(data.description||'').trim(),
                fileDataUrl: data.fileDataUrl || null, fileName: data.fileDataUrlName || null, status: m?.status || '待簽核',
                uid: u.uid, dateStr: String(data.number||'').slice(0,10), updatedAt: fns.serverTimestamp(), createdAt: m?.createdAt || fns.serverTimestamp(),
              };
              if (!isEdit) {
                let newId = null; try { const ref = await withRetry(() => fns.addDoc(fns.collection(db, 'memos'), payload)); newId = ref?.id || null; } catch {}
                const arr = readLocal(u.uid); arr.push({ id: newId || id(), ...payload }); writeLocal(u.uid, arr);
              } else if (m?.id) {
                try { await withRetry(() => fns.updateDoc(fns.doc(db, 'memos', m.id), payload)); } catch {}
                const arr = readLocal(u.uid); const idx = arr.findIndex((x) => x.id === m.id); if (idx >= 0) { arr[idx] = { ...arr[idx], ...payload }; writeLocal(u.uid, arr); }
              }
              render();
              return true;
            } catch (e) { alert(`儲存失敗：${e?.message || e}`); return false; }
          },
        });
      };
      const doDelete = async (m) => {
        openModal({ title: '刪除簽呈', message: `確定要刪除「${m.number || ''}」嗎？`, fields: [], submitText: '刪除', onSubmit: async () => {
          try { await ensureFirebase(); const u = auth?.currentUser || null; if (!u) return false; try { if (db && fns.deleteDoc && fns.doc) { await withRetry(() => fns.deleteDoc(fns.doc(db, 'memos', m.id))); } } catch {} const arr = readLocal(u.uid).filter((x) => x.id !== m.id); writeLocal(u.uid, arr); render(); return true; } catch (e) { alert(`刪除失敗：${e?.message || e}`); return false; }
        }});
      };
      const doSupplement = async (m) => {
        openModal({ title: '補件', fields: [ { key:'number', label:'編號', type:'text', readonly:true }, { key:'fileDataUrl', label:'上傳文件', type:'file', accept:'.pdf,.png,.jpg,.jpeg' } ], initial: { number: m.number }, submitText: '送出', onSubmit: async (data) => {
          try { await ensureFirebase(); const u = auth?.currentUser || null; if (!u) return false; const payload = { fileDataUrl: data.fileDataUrl || null, status: '待簽核', updatedAt: fns.serverTimestamp() }; try { await withRetry(() => fns.updateDoc(fns.doc(db, 'memos', m.id), payload)); } catch {} const arr = readLocal(u.uid); const idx = arr.findIndex((x) => x.id === m.id); if (idx >= 0) { arr[idx] = { ...arr[idx], ...payload }; writeLocal(u.uid, arr); } render(); return true; } catch (e) { alert(`補件失敗：${e?.message || e}`); return false; }
        }});
      };
      const render = async () => {
        try {
          await ensureFirebase(); const u = auth?.currentUser || null; if (!u) { listEl.innerHTML = `<tr><td colspan="6">尚未登入</td></tr>`; return; }
          let arr = [];
          try {
            if (db && fns.collection && fns.getDocs && fns.query && fns.where) {
              const q = fns.query(fns.collection(db, 'memos'), fns.where('uid','==',u.uid));
              const snap = await withRetry(() => fns.getDocs(q));
              snap.forEach((doc) => { const d = doc.data(); arr.push({ id: doc.id, ...d }); });
            }
          } catch {}
          if (!arr.length) { arr = readLocal(u.uid); }
          if (!arr.length) { listEl.innerHTML = `<tr><td colspan="6">尚無資料</td></tr>`; return; }
          const frag = document.createDocumentFragment();
          arr.sort((a,b) => String(b.number||'').localeCompare(String(a.number||'')));
          arr.forEach((m) => {
            const row = document.createElement('tr');
            const c1 = document.createElement('td'); c1.textContent = m.number || '';
            const c2 = document.createElement('td'); c2.textContent = m.subject || '';
            const c3 = document.createElement('td'); c3.textContent = m.description || '';
            const c4 = document.createElement('td'); const btnDoc = document.createElement('button'); btnDoc.className = 'btn'; btnDoc.textContent = '查看'; attachPressInteractions(btnDoc); btnDoc.addEventListener('click', () => openViewDoc(m)); c4.appendChild(btnDoc);
            const c5 = document.createElement('td'); c5.textContent = m.status || '待簽核';
            const c6 = document.createElement('td'); c6.className = 'cell-actions';
            const btnEdit = document.createElement('button'); btnEdit.className = 'btn btn-grey'; btnEdit.textContent = '編輯'; attachPressInteractions(btnEdit); btnEdit.addEventListener('click', () => addOrEdit(m,'edit'));
            const btnDel = document.createElement('button'); btnDel.className = 'btn btn-grey'; btnDel.textContent = '刪除'; attachPressInteractions(btnDel); btnDel.addEventListener('click', () => doDelete(m));
            c6.appendChild(btnEdit); c6.appendChild(btnDel);
            if (String(m.status||'') === '請補件') { const btnSup = document.createElement('button'); btnSup.className = 'btn btn-darkgrey'; btnSup.textContent = '補件'; attachPressInteractions(btnSup); btnSup.addEventListener('click', () => doSupplement(m)); c6.appendChild(btnSup); }
        ...
        } catch (e) { listEl.innerHTML = `<tr><td colspan="6">尚無資料</td></tr>`; }
      };
      const renderOrg = async () => {
        try {
          await ensureFirebase(); const u = auth?.currentUser || null; if (!u) { listOrgEl.innerHTML = `<tr><td colspan="6">尚未登入</td></tr>`; return; }
          const me = appState.accounts.find((a) => a.id === (appState.currentUserId || u.uid)) || null;
          const meCompanyIds = Array.isArray(me?.companyIds) ? me.companyIds : (me?.companyId ? [me.companyId] : []);
          if (!meCompanyIds.length) { listOrgEl.innerHTML = `<tr><td colspan="6">尚無公司資訊</td></tr>`; return; }
          const coSet = new Set(meCompanyIds.map((x) => String(x||'')));
          let arr = [];
          try {
            if (db && fns.collection && fns.getDocs) {
              const snap = await withRetry(() => fns.getDocs(fns.collection(db, 'memos')));
              snap.forEach((doc) => { const d = doc.data(); arr.push({ id: doc.id, ...d }); });
            }
          } catch {}
          const accountsAll = Array.isArray(appState.accounts) ? appState.accounts : [];
          const filtered = arr.filter((m) => {
            const acc = accountsAll.find((a) => String(a.uid||a.id||'') === String(m.uid||'')) || null;
            const ids = acc ? (Array.isArray(acc.companyIds) ? acc.companyIds : (acc.companyId ? [acc.companyId] : [])) : [];
            return ids.some((id0) => coSet.has(String(id0||'')));
          });
          if (!filtered.length) { listOrgEl.innerHTML = `<tr><td colspan="6">尚無資料</td></tr>`; return; }
          const frag = document.createDocumentFragment();
          filtered.sort((a,b) => String(b.number||'').localeCompare(String(a.number||'')));
          filtered.forEach((m) => {
            const row = document.createElement('tr');
            const c1 = document.createElement('td'); c1.textContent = m.number || '';
            const c2 = document.createElement('td'); c2.textContent = m.subject || '';
          ...
          listOrgEl.innerHTML = ''; listOrgEl.appendChild(frag);
        } catch (e) { listOrgEl.innerHTML = `<tr><td colspan="6">尚無資料</td></tr>`; }
      };
      btnAdd?.addEventListener('click', () => addOrEdit(null,'new'));
      render();
      renderOrg();
    } else if (label === '推播') {
      container.innerHTML = `
...
function renderNetworkIndicator() {
  ...
  if (netIndicatorHome) {
    netIndicatorHome.className = "net-indicator " + (navigator.onLine ? "net-online" : "net-offline");
    netIndicatorHome.setAttribute("title", navigator.onLine ? "有網路" : "無網路");
    netIndicatorHome.setAttribute("aria-label", navigator.onLine ? "有網路" : "無網路");
  }
}
