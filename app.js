...
(async()=>{db=await openIDB();await loadFirebase();if(firebaseConfigJson){firebaseConfigJson.value=localStorage.getItem('firebaseConfig')||''}await refreshAuthState();updateNavVisibility();await renderPoints();await renderHistory();setActiveTab('login')})();
...
async function renderAccounts(){if(!accountsList)return;const items=await getAll('accounts');accountsList.innerHTML='';items.forEach(a=>{const el=document.createElement('div');el.className='item';const svc=(a.serviceCommunities||[]).join(',');const left=`<div><strong>${a.name}</strong> (${a.role})<br><span>${a.phone} • ${a.email}</span><br><span>社區：${svc}</span></div>`;const right=`<div><button class="btn" data-act="edit" data-id="${a.id}">編輯</button> <button class="btn outline" data-act="del" data-id="${a.id}">刪除</button></div>`;el.innerHTML=left+right;el.querySelector('[data-act="edit"]').addEventListener('click',()=>openAccountModal('edit',a));el.querySelector('[data-act="del"]').addEventListener('click',()=>{if(!confirm(`確定要刪除帳號 ${a.name}？`))return;tx('accounts','readwrite').delete(a.id).onsuccess=()=>{renderAccounts()}});accountsList.appendChild(el)})}

function openCommunityModal(mode,data){modalCommunity.classList.remove('hidden');modalCommunity.dataset.mode=mode||'create';mCommCode.disabled=mode==='edit';mCommCode.value=data?data.code:'';mCommName.value=data?data.name:'';mCommArea.value=data?data.area:''}
...
if(btnSaveCommunity){btnSaveCommunity.addEventListener('click',()=>{const code=mCommCode.value.trim();const name=mCommName.value.trim();const area=mCommArea.value.trim();if(!code||!name||!area)return;tx('communities','readwrite').put({code,name,area,updatedAt:Date.now()}).onsuccess=()=>{closeCommunityModal();renderCommunities()}})}
